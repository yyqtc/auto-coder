...
class TestGetDriveLetter:
    ...
    def test_get_drive_letter_windows(self):
        ...
        with patch("execute_replan_utils.platform.system", return_value="Windows"), \
...
class TestExecuteScriptSubprocess:
    ...
    def test_execute_script_windows_success(self, mock_config):
        """测试 Windows 系统成功执行"""
        with patch("execute_replan_utils.platform.system", return_value="Windows"), \
             patch("execute_replan_utils.config", mock_config), \
...
class TestAnalyzeWhatToDo:
    ...
    def test_analyze_what_to_do_basic(self, mock_config):
        """测试基本分析功能"""
        with patch("execute_replan_utils.config", mock_config), \
             patch("execute_replan_utils._execute_script_subprocess") as mock_execute, \
    ...
    def test_analyze_what_to_do_windows_execute_path(self, mock_config):
        ...
        with patch("execute_replan_utils.platform.system", return_value="Windows"), \
             patch("execute_replan_utils.config", mock_config), \
             patch("execute_replan_utils._execute_script_subprocess") as mock_execute, \
             patch("execute_replan_utils.tempfile.NamedTemporaryFile") as mock_temp, \
             patch("os.path.exists", return_value=False):
