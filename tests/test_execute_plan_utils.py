...
class TestConvertDocxToMarkdown:
    """测试 convert_docx_to_markdown 函数"""
    ...
    def test_convert_docx_invalid_extension(self):
        """测试非 docx 文件"""
    ...
    @pytest.mark.skipif(not os.path.exists("tests/test_data"), reason="需要测试数据")
    def test_convert_docx_success(self, temp_dir):
        """测试成功转换 DOCX 文件（需要实际的 docx 文件）"""
        # 这个测试需要实际的 docx 文件，所以标记为可选
        pass


class TestConvertPdfToMarkdown:
    """测试 convert_pdf_to_markdown 函数"""
